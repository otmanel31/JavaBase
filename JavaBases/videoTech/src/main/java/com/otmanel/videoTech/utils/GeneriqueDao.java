...
import com.otmanel.videoTech.utils.QueryBuilder.TypeWhere;
	...
	public static class GenericDaoBuilder<T>{
		...
		private void fillFieldInfos() {
			...
			for( Method mgetter : methods) {
				...
				if( mgetter.getName().matches("^get[A-Z][A-Za-z0-9_]+$") && mgetter.getParameterTypes().length == 0 
						&&( mgetter.getReturnType().equals(int.class) || mgetter.getReturnType().equals(double.class)
						|| mgetter.getReturnType().equals(Boolean.class) || mgetter.getReturnType().equals(String.class))) {
